...
\begin{frame}{Algoritmul Distribuit (SPMD)}
  \begin{block}{Tranziția la Sistem Distribuit}
    Spre deosebire de algoritmul serial, varianta \textbf{SPMD} (Single Program, Multiple Data) presupune noduri care rulează aceeași logică de protocol, dar operează pe date diferite.
  \end{block}

  \vspace{0.3cm}

  \textbf{Fluxul celor 3 faze (Mesaje în Rețea):}
  \begin{enumerate}
    \item \textbf{AS Exchange:} Obținere TGT.
    \item \textbf{TGS Exchange:} Obținere Tichet de Serviciu.
    \item \textbf{AP Exchange:} Accesare Resursă.
  \end{enumerate}
\end{frame}

\begin{frame}{Faza 1 (SPMD): Autentificarea Inițială (AS Exchange)}
...
\begin{frame}{Faza 2 (SPMD): Obținerea Accesului (TGS Exchange)}
...
\begin{frame}{Faza 3 (SPMD): Accesarea Serviciului (Client-Server)}
...
\begin{frame}{Analiza Scalabilității (Variația Nodurilor)}
  \small

  \begin{columns}[t] % Aliniere la partea de sus

    % --- COLOANA STÂNGA (Avantaje - Citire) ---
    \begin{column}{0.48\textwidth}
      \begin{block}{Scalabilitate la Citire (Autentificare)}
        \begin{itemize}
          \item \textbf{Creștere Liniară:} Adăugarea de noduri \textit{Slave} multiplică direct capacitatea de procesare a cererilor de logare.
          \item \textbf{Performanță:} Sistemul poate susține mii de cereri simultane prin balansarea încărcării între Slave-uri.
        \end{itemize}
      \end{block}
    \end{column}

    % --- COLOANA DREAPTA (Limitări - Scriere) ---
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{Limitare la Scriere (Modificări)}
        \begin{itemize}
          \item \textbf{Master Exclusiv:} Doar KDC-ul Master are drept de scriere în baza de date.
          \item \textbf{SPOF:} Master-ul rămâne un \textit{Single Point of Failure} pentru operațiuni administrative (ex: schimbarea parolei).
        \end{itemize}
      \end{alertblock}
    \end{column}
  ...
  % --- BLOC FINAL (Propagation Lag) ---
  \begin{exampleblock}{Fenomen: Propagation Lag}
    Există o întârziere inerentă între momentul scrierii pe Master și actualizarea tuturor Slave-urilor.

    \textit{Consecință:} Un utilizator ar putea folosi vechea parolă pe un Slave neactualizat timp de câteva secunde/minute.
  \end{exampleblock}

\end{frame}

\begin{frame}{Demo Live și Concluzii}
  \small

  % 1. Setup-ul Demo-ului (Arhitectura)
  \begin{block}{Arhitectura Implementării (Sistem Distribuit SPMD)}
  ...
  % 2. Scenariul de Test (Fluxul din UI)
  \begin{columns}[c]
    \begin{column}{0.55\textwidth}
      \textbf{Fluxul de Demonstrație (Step-by-Step):}
      \begin{enumerate}
        \item \textbf{Login (AS Exchange):}
              UI $\rightarrow$ KDC. Obținere TGT (parola rămâne locală).
        \item \textbf{Get Service Ticket (TGS Exchange):}
              Folosire TGT $\rightarrow$ Obținere tichet pentru \texttt{:9090}.
        \item \textbf{Access Resource (AP Exchange):}
    ...
    \begin{column}{0.45\textwidth}
      \begin{alertblock}{Insight Cheie}
        \textbf{API Server NU contactează KDC!}

        Verificarea tichetului se face \textbf{offline} (folosind cheia secretă partajată) și Replay Cache.
      \end{alertblock}
    \end{column}
  \end{columns}
  ...
  \vspace{0.3cm}
    ...
    \textbf{Concluzie:} Kerberos permite autentificare sigură pe rețele nesigure, fără a expune secretele.
  ...
  \begin{frame}{Topologie și Workflow Complet}
    ...
    \begin{figure}
      ...
      % Scalăm imaginea astfel încât să încapă pe slide, păstrând proporțiile

  

      % 0.8\textheight lasă loc și pentru titlu și caption

  

      \includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{images/topology_and_workflow.png}
